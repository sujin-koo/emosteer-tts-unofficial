...
class DiTBlock(nn.Module):
    def __init__(
        ...
        self.alpha = 1.0   # steering intensity
    ...
    def forward(self, x, t, mask=None, rope=None):  # x: noised input, t: time embedding
        ...
        # (A) SAVE FIRST RESIDUAL
        if self.save_residual:
            self.first_residual = x_res1.detach()

        ###
        if self.steer_vector is not None:
            # steer_vector shape: [B, T_steer, D]
            # x_res1 shape: [B, T_block, D]

            if self.steer_vector.shape[1] != x_res1.shape[1]:
                steer = F.interpolate(
                    self.steer_vector.transpose(1, 2),
                    size=x_res1.shape[1],
                    mode='nearest'
                ).transpose(1, 2)
            else:
                steer = self.steer_vector

            # === Steering with per-token renormalization (Eq. 8) ===
            # steer: [B, T, D], x_res1: [B, T, D]
            steered = x_res1 + self.alpha * steer

            # Per-token L2 renormalization (preserve original norm per token)
            # orig_norm: [B, T, 1], new_norm: [B, T, 1]
            orig_norm = x_res1.norm(p=2, dim=-1, keepdim=True)  # [B, T, 1]
            new_norm = steered.norm(p=2, dim=-1, keepdim=True)  # [B, T, 1]
            x_res1 = steered * (orig_norm / (new_norm + 1e-8))

            # Debug info (only print once per forward pass to avoid spam)
            if not hasattr(self, '_debug_printed'):
                steer_magnitude = (self.alpha * steer).norm(p=2).item()
                x_magnitude = x_res1.norm(p=2).item()
                print(f"[DEBUG] Steering applied: alpha={self.alpha:.3f}, steer_mag={steer_magnitude:.4f}, x_mag={x_magnitude:.4f}")
                self._debug_printed = True


        norm = self.ff_norm(x_res1) * (1 + scale_mlp[:, None]) + shift_mlp[:, None]
